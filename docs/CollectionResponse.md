...
**schedule** | [**CollectionScheduleConfig**](CollectionScheduleConfig.md) | NOT REQUIRED. Schedule configuration for automatic re-processing. When set, a COLLECTION_TRIGGER trigger is created and linked to this collection. | [optional] 
**trigger_id** | **str** | NOT REQUIRED. ID of the linked trigger for scheduled re-processing. Automatically set when a schedule is configured. | [optional] 
...
**lifecycle_state** | **str** | Storage lifecycle state: &#39;active&#39; (Qdrant + S3), &#39;cold&#39; (S3 only), &#39;archived&#39; (metadata only). Managed via lifecycle API. | [optional] [default to 'active']
**s3_vector_index** | **str** | S3 Vectors index name for this collection (e.g. &#39;col_{collection_id}&#39;). | [optional] 
**last_lifecycle_transition** | **datetime** | Timestamp of the most recent lifecycle state change. | [optional] 
**tiering_rules** | [**List[TieringRule]**](TieringRule.md) | Automatic storage tiering rules (not enforced in V1). | [optional] 
...
**alert_applications** | [**List[AlertApplicationConfigOutput]**](AlertApplicationConfigOutput.md) | NOT REQUIRED. List of alerts to automatically execute when documents are ingested. Each entry specifies: alert_id, execution_mode, input_mappings, execution_phase, priority. Alerts run retrievers on ingested documents and send notifications when matches are found. Empty/null if no alerts attached. Use for: Content monitoring, safety detection, compliance alerts. | [optional] 
**retriever_enrichments** | [**List[RetrieverEnrichmentConfigOutput]**](RetrieverEnrichmentConfigOutput.md) | NOT REQUIRED. List of retriever enrichments to run on documents during post-processing. Each entry specifies: retriever_id, input_mappings, write_back_fields, execution_phase, priority. Retriever enrichments execute a retriever pipeline and write selected result fields back to documents. Empty/null if no enrichments attached. Use for: LLM classification, cross-collection joins, enrichment. | [optional] 
**taxonomy_count** | **int** | Number of taxonomies connected to this collection | [optional] 
**retriever_count** | **int** | Number of retrievers connected to this collection | [optional] 
