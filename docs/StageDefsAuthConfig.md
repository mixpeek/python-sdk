...
------------ | ------------- | ------------- | -------------
**type** | [**StageDefsAuthType**](StageDefsAuthType.md) | REQUIRED. Authentication method to use. Options: none (public API), api_key (API keys), bearer (OAuth/JWT), basic (HTTP Basic Auth), custom_header (non-standard headers). See AuthType enum for detailed description of each type. | [optional] 
