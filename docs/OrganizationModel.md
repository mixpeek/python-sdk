...
**auth_provider_org_id** | **str** | External auth provider&#39;s organization identifier (e.g., PropelAuth org ID). Used as a stable lookup key that doesn&#39;t depend on organization name matching. Set at org creation time and never changes. | [optional] 
...
**max_active_batches** | **int** | Maximum number of concurrently active (PENDING/IN_PROGRESS/PROCESSING) batches. Prevents runaway compute costs from too many simultaneous batch jobs. When null, uses tier-based defaults: Free&#x3D;1, Pro&#x3D;3, Team&#x3D;5, Enterprise&#x3D;10. Set explicitly to override the tier default. | [optional] 
