...
class PublicRetrieverAPIApi:
    ...
    def execute_retriever_name(
        ...
        x_public_api_key: Optional[StrictStr] = None,
        ...
        Execute a published retriever (public endpoint).  **Authentication:** - API key is OPTIONAL for public retrievers - Supports: no key, prk_ keys (deprecated), or ret_sk_ keys - If password-protected, requires `X-Retriever-Password` header  **Rate Limiting:** - Subject to per-retriever rate limits (per minute/hour/day) - May also have IP-based rate limits  **Response:** - Only returns fields specified in `exposed_fields` configuration - Internal metadata is stripped from results - Includes `execution_id` for interaction tracking - Presigned URLs returned by default (return_presigned_urls=true) for media rendering  **Example (no API key - recommended for public access):** ```bash curl -X POST \"https://api.mixpeek.com/v1/public/retrievers/video-search/execute\" \\   -H \"Content-Type: application/json\" \\   -d '{     \"inputs\": {\"query\": \"red car\"},     \"pagination\": {\"method\": \"offset\", \"page_number\": 1, \"page_size\": 10}   }' ```  **Example with ret_sk_ key (for SDK/programmatic access):** ```bash curl -X POST \"https://api.mixpeek.com/v1/public/retrievers/video-search/execute\" \\   -H \"X-Public-API-Key: ret_sk_abc123...\" \\   -H \"Content-Type: application/json\" \\   -d '{     \"inputs\": {\"query\": \"red car\"},     \"pagination\": {\"method\": \"offset\", \"page_number\": 1, \"page_size\": 10}   }' ```
    ...
    def list_retrievers(
        ...
        search: Annotated[Optional[Annotated[str, Field(strict=True, max_length=200)]], Field(description="Search query for filtering by title or description")] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
