...
class DocumentGraphExtractorParams(BaseModel):
    ...
    vertical_threshold: Optional[Union[Annotated[float, Field(le=100, strict=True, ge=1)], Annotated[int, Field(le=100, strict=True, ge=1)]]] = Field(default=15, description="Maximum vertical gap (in points) between lines to be grouped in same block. Increase for looser grouping, decrease for tighter blocks. Default 15pt works well for standard documents.")
    horizontal_threshold: Optional[Union[Annotated[float, Field(le=200, strict=True, ge=1)], Annotated[int, Field(le=200, strict=True, ge=1)]]] = Field(default=50, description="Maximum horizontal distance (in points) for overlap detection. Affects column detection and block merging. Increase for wider columns, decrease for narrow layouts.")
    ...
    base_confidence: Optional[Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = Field(default=0.85, description="Base confidence score for embedded (native) text. Penalties are subtracted for OCR artifacts, encoding issues, etc.")
    min_confidence_for_vlm: Optional[Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = Field(default=0.6, description="Confidence threshold below which VLM correction is triggered. Blocks with confidence < this value get sent to VLM for correction. Only applies when use_vlm_correction=True.")
    ...
    vlm_model: Optional[StrictStr] = Field(default='gemini-2.5-flash', description="Specific model for VLM correction. Examples: 'gemini-2.5-flash', 'gpt-4o', 'claude-3-5-sonnet'.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "vertical_threshold": obj.get("vertical_threshold") if obj.get("vertical_threshold") is not None else 15,
            "horizontal_threshold": obj.get("horizontal_threshold") if obj.get("horizontal_threshold") is not None else 50,
