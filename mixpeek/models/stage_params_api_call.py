...
class StageParamsApiCall(BaseModel):
    ...
    auth: Optional[StageDefsAuthConfig] = Field(default=None, description="OPTIONAL. Authentication configuration. Uses organization vault for credential storage. See AuthConfig for details.")
    ...
    rate_limit: Optional[StageDefsRateLimitConfig] = Field(default=None, description="OPTIONAL. Rate limiting configuration per domain.")
    ...
    when: Optional[StageDefsLogicalOperator] = Field(default=None, description="OPTIONAL. Conditional filter for selective enrichment. Only documents matching condition will call API. RECOMMENDED for cost/performance optimization.")
    on_error: Optional[StageDefsErrorHandling] = Field(default=None, description="Error handling strategy: 'skip': Pass document through unchanged. 'remove': Remove failed documents. 'raise': Halt pipeline on error. Default: 'skip'.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "auth": StageDefsAuthConfig.from_dict(obj["auth"]) if obj.get("auth") is not None else None,
            ...
            "rate_limit": StageDefsRateLimitConfig.from_dict(obj["rate_limit"]) if obj.get("rate_limit") is not None else None,
