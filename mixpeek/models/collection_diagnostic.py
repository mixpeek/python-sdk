...
class CollectionDiagnostic(BaseModel):
    ...
    document_count: Optional[StrictInt] = Field(default=0, description="Number of documents in collection")
    expected_documents: Optional[StrictInt] = Field(default=None, description="Expected document count")
    status: StrictStr = Field(description="Collection status (ready, processing, empty)")
    __properties: ClassVar[List[str]] = ["collection_id", "collection_name", "document_count", "expected_documents", "status"]
