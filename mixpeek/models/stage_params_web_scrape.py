...
class StageParamsWebScrape(BaseModel):
    ...
    max_depth: Optional[Annotated[int, Field(le=5, strict=True, ge=0)]] = Field(default=None, description="OPTIONAL. Maximum link depth to follow from seed URL. None = single URL mode (default behavior). 0 = only seed page. 1 = seed + direct links from seed. 2 = seed + links + links from those pages. Higher values exponentially increase crawl time. Recommended: 1-2 for most use cases.")
    max_pages: Optional[Annotated[int, Field(le=100, strict=True, ge=1)]] = Field(default=None, description="OPTIONAL. Maximum total pages to crawl. None = single URL mode (default behavior). Crawling stops when this limit is reached regardless of depth. Acts as a safety limit to prevent runaway crawls. Range: 1-100.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "url_patterns": StageDefsURLPatterns.from_dict(obj["url_patterns"]) if obj.get("url_patterns") is not None else None,
