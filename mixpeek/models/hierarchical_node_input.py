...
class HierarchicalNodeInput(BaseModel):
    """
    A node in a hierarchical taxonomy (explicit representation).
    """ # noqa: E501
    collection_id: StrictStr = Field(description="The ID of the collection representing this node in the hierarchy.")
    parent_collection_id: Optional[StrictStr] = Field(default=None, description="The ID of the parent collection in the hierarchy (``None`` for root nodes).")
    label: Optional[StrictStr] = Field(default=None, description="Optional human-readable label for the node")
    summary: Optional[StrictStr] = Field(default=None, description="Optional summary describing the node")
    keywords: Optional[List[StrictStr]] = Field(default=None, description="Optional keywords associated with the node")
    retriever_id: Optional[StrictStr] = Field(default=None, description="Optional per-node retriever. If omitted, taxonomy-level `retriever_id` (if provided) is used.")
    enrichment_fields: Optional[List[EnrichmentField]] = Field(default=None, description="Optional field-level enrichment behaviour for this specific node.")
    input_mappings: Optional[List[InputMapping]] = Field(default=None, description="Optional override of the taxonomy-level input_mappings when this node is matched.")
