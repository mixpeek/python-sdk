...
class CourseContentExtractorParams(BaseModel):
    ...
    run_visual_embedding: Optional[StrictBool] = Field(default=True, description="Generate SigLIP visual embeddings (768D) for video frames.")
    run_structure_embedding: Optional[StrictBool] = Field(default=True, description="Generate DINOv2 visual structure embeddings (768D) for layout comparison.")
    ...
    run_vlm_frame_analysis: Optional[StrictBool] = Field(default=False, description="Run VLM on video frame thumbnails to extract structured fields: frame_type, page_context, ui_labels, workflow_steps, config_options. Enables drift detection and UI comparison use cases.")
    vlm_provider: Optional[StrictStr] = Field(default='google', description="VLM provider: 'google' (Gemini API) or 'vllm' (local GPU with Qwen2.5-VL).")
    vlm_model: Optional[StrictStr] = Field(default='gemini-2.5-flash', description="VLM model. For google: 'gemini-2.5-flash'. For vllm: 'Qwen/Qwen2.5-VL-7B-Instruct'.")
    ...
    def vlm_provider_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['google', 'vllm']):
            raise ValueError("must be one of enum values ('google', 'vllm')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "run_visual_embedding": obj.get("run_visual_embedding") if obj.get("run_visual_embedding") is not None else True,
            "run_structure_embedding": obj.get("run_structure_embedding") if obj.get("run_structure_embedding") is not None else True,
            ...
            "run_vlm_frame_analysis": obj.get("run_vlm_frame_analysis") if obj.get("run_vlm_frame_analysis") is not None else False,
            "vlm_provider": obj.get("vlm_provider") if obj.get("vlm_provider") is not None else 'google',
            "vlm_model": obj.get("vlm_model") if obj.get("vlm_model") is not None else 'gemini-2.5-flash',
