...
class InputMappingSource(BaseModel):
    """
    ...
    """ # noqa: E501
    source_type: StrictStr = Field(description="Where the value comes from")
    path: Optional[StrictStr] = Field(default=None, description="JSONPath to document field (when source_type='document_field')")
    value: Optional[Any] = None
    __properties: ClassVar[List[str]] = ["source_type", "path", "value"]

    @field_validator('source_type')
    def source_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['document_field', 'constant']):
            raise ValueError("must be one of enum values ('document_field', 'constant')")
        return value
