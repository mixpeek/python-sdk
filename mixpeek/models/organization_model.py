...
class OrganizationModel(BaseModel):
    ...
    auth_provider_org_id: Optional[StrictStr] = Field(default=None, description="External auth provider's organization identifier (e.g., PropelAuth org ID). Used as a stable lookup key that doesn't depend on organization name matching. Set at org creation time and never changes.")
    ...
    max_active_batches: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="Maximum number of concurrently active (PENDING/IN_PROGRESS/PROCESSING) batches. Prevents runaway compute costs from too many simultaneous batch jobs. When null, uses tier-based defaults: Free=1, Pro=3, Team=5, Enterprise=10. Set explicitly to override the tier default.")
    ...
    __properties: ClassVar[List[str]] = ["internal_id", "organization_id", "auth_provider_org_id", "organization_name", "logo_url", "account_type", "credit_count", "free_tier_usage_limit", "metadata", "billing_email", "stripe_customer_id", "default_payment_method_id", "auto_billing_enabled", "billing_cycle_start", "current_month_usage", "last_invoice_date", "billing_period_start", "monthly_spending_budget", "spending_alert_thresholds", "spending_alerts_enabled", "spending_alerts_sent", "hard_spending_cap", "hard_cap_enabled", "rate_limits", "created_at", "updated_at", "users", "infrastructure", "max_dedicated_clusters", "max_active_batches", "dedicated_cluster_ids", "secrets", "audit_settings"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "max_active_batches": obj.get("max_active_batches"),
