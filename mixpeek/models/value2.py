...
VALUE2_ANY_OF_SCHEMAS = ["List[List[float]]", "List[float]", "str"]
...
class Value2(BaseModel):
    ...
    if TYPE_CHECKING:
        actual_instance: Optional[Union[List[List[float]], List[float], str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "List[List[float]]", "List[float]", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Value2 with anyOf schemas: List[List[float]], List[float], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Value2 with anyOf schemas: List[List[float]], List[float], str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[List[float]], List[float], str]]:
