...
class SourceCollectionInput(BaseModel):
    """
    A source collection for a flat taxonomy.
    """ # noqa: E501
    collection_id: StrictStr = Field(description="The ID of the source collection for the taxonomy.")
    enrichment_fields: Optional[List[EnrichmentField]] = Field(default=None, description="Fields to copy from matched taxonomy node when enriching (append/replace semantics). If omitted, the full payload is copied.")
    __properties: ClassVar[List[str]] = ["collection_id", "enrichment_fields"]
