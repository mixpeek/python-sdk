...
class StageDefsLearnedFusionConfig(BaseModel):
    """
    ...
    exploration_bonus: Optional[Union[Annotated[float, Field(le=10, strict=True, ge=0.1)], Annotated[int, Field(le=10, strict=True, ge=1)]]] = Field(default=1, description="Controls exploration vs exploitation balance. 1.0 = balanced (default). Higher = more exploration (try diverse weights). Lower = more exploitation (use known winners). Typical range: 0.5-2.0.")
    prior_alpha: Optional[Union[Annotated[float, Field(strict=True, ge=0.1)], Annotated[int, Field(strict=True, ge=1)]]] = Field(default=1, description="Beta distribution prior for positive feedback (α). 1.0 = uniform prior (no initial bias). Higher = initial belief that features are effective.")
    prior_beta: Optional[Union[Annotated[float, Field(strict=True, ge=0.1)], Annotated[int, Field(strict=True, ge=1)]]] = Field(default=1, description="Beta distribution prior for negative feedback (β). 1.0 = uniform prior (no initial bias). Higher = initial belief that features are ineffective.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "exploration_bonus": obj.get("exploration_bonus") if obj.get("exploration_bonus") is not None else 1,
            "prior_alpha": obj.get("prior_alpha") if obj.get("prior_alpha") is not None else 1,
            "prior_beta": obj.get("prior_beta") if obj.get("prior_beta") is not None else 1
        })
        return _obj
