...
class StageParamsLlmEnrich(BaseModel):
    ...
    """ # noqa: E501
    provider: Optional[StageDefsLLMProvider] = Field(default=None, description="LLM provider to use. Supported providers: - openai: GPT models (GPT-4o, GPT-4o-mini) - google: Gemini models (Gemini 2.5 Flash) - anthropic: Claude models (Claude 3.5 Sonnet/Haiku)  If not specified, defaults to 'google'. Can be auto-inferred from model_name.")
    model_name: Optional[StrictStr] = Field(default='null', description="Specific LLM model to use. If not specified, uses provider default. Examples: gemini-2.5-flash, gpt-4o-mini, claude-3-5-haiku-20241022")
    inference_name: Optional[StrictStr] = Field(default='null', description="DEPRECATED: Use 'provider' and 'model_name' instead. Legacy format: 'provider:model' (e.g., 'gemini:gemini-2.5-flash'). Kept for backward compatibility only.")
    ...
    max_tokens: Optional[Annotated[int, Field(le=16000, strict=True, ge=100)]] = Field(default=None, description="Maximum output tokens for the LLM response. If not specified, uses the provider default (4000). Increase this if output is being truncated. Note: Gemini counts tokens more aggressively than Claude/GPT — a 1000-token limit may produce only ~350 characters with Gemini.")
    temperature: Optional[Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = Field(default=0.2, description="Sampling temperature passed to the LLM.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "max_tokens": obj.get("max_tokens") if obj.get("max_tokens") is not None else null,
            "temperature": obj.get("temperature") if obj.get("temperature") is not None else 0.2,
            "api_key": obj.get("api_key"),
            "use_vcache": obj.get("use_vcache") if obj.get("use_vcache") is not None else False,
