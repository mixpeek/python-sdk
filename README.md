...
The official Python SDK for the [Mixpeek](https://mixpeek.com) multimodal data processing and retrieval API.
...
mp = Mixpeek("sk_your_api_key", namespace="ns_your_namespace")

# Search across any collection with one line
results = mp.search("Q4 compliance discussion", collection="meetings")

# Index files from cloud storage
mp.index("s3://content-library/videos/", collection="meetings")
```
...
|-----------|-------------|---------|
| `api_key` | `MIXPEEK_API_KEY` | *required* |
| `namespace` | `MIXPEEK_NAMESPACE` | `None` |
...
mp = Mixpeek(api_key="sk_xxx")

# Or use environment variables:
# export MIXPEEK_API_KEY=sk_xxx
...
Every resource is available as a manager with standard CRUD methods:

### Namespaces

```python
mp.namespaces.list()
mp.namespaces.create(name="my-namespace", feature_extractors=[...])
mp.namespaces.get("ns_xxx")
mp.namespaces.delete("ns_xxx")
```
...
```python
mp.buckets.list()
mp.buckets.create(name="my-bucket")
mp.buckets.upload("bkt_xxx", url="s3://path/to/file.mp4", metadata={"key": "value"})
mp.buckets.list_objects("bkt_xxx")
mp.buckets.delete("bkt_xxx")
```
...
```python
mp.collections.list()
mp.collections.create(
    name="my-collection",
    feature_extractor={"type": "multimodal"},
    source={"bucket_id": "bkt_xxx"},
)
mp.collections.trigger("col_xxx")
mp.collections.delete("col_xxx")
```
...
```python
mp.retrievers.list()
mp.retrievers.create(name="my-retriever", stages=[...])
mp.retrievers.execute("ret_xxx", inputs={"query": "test"}, settings={"limit": 10})
mp.retrievers.delete("ret_xxx")
```

### Documents

```python
mp.documents.list("col_xxx")
mp.documents.get("col_xxx", "doc_xxx")
mp.documents.update("col_xxx", "doc_xxx", update_data={"field": "value"})
mp.documents.batch_update("col_xxx", updates=[{"document_id": "doc_xxx", "update_data": {...}}])
mp.documents.search(collection_ids=["col_xxx"], query="test")
mp.documents.delete("col_xxx", "doc_xxx")
```

## Error Handling

```python
from mixpeek._client.client import MixpeekAPIError

try:
    mp.search("test", collection="nonexistent")
except MixpeekAPIError as e:
    print(e.status)   # 404
...
The high-level client wraps a fully-typed OpenAPI-generated SDK. For complete control, use the generated classes directly:
...
with mixpeek.ApiClient(configuration) as api_client:
    api = mixpeek.CollectionsApi(api_client)
